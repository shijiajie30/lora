...
| 格式 | CSV 文件 | Arrow 缓存 |
| 加载速度 | 快 | 很快 |
| 训练时间 | 约 3-4 小时 (M2 Max) | 约 12-18 小时 (M2 Max) |
| 磁盘占用 | ~5 MB | ~46 MB |

**优势**:
- ✅ **样本数较少**，训练更快（约为原数据集的 1/3）
- ✅ **适合快速实验**和调试
- ✅ **文件小**，易于编辑和查看
- ✅ **格式简单**，便于自定义数据
...
### CSV 数据集 (6,724 样本，3 epochs)

| 设备 | 配置 | 预计时间 |
|------|------|---------|
| **CUDA GPU** | | |
| RTX 3090 (24GB) | batch=2, acc=8, 4-bit | 3-4 小时 |
| RTX 4090 (24GB) | batch=4, acc=4, 4-bit | 2-3 小时 |
| A100 (40GB) | batch=4, acc=4, 4-bit | 1.5-2 小时 |
| **Apple Silicon** | | |
| M1 Pro (16GB) | batch=1, len=384 | 8-12 小时 |
| M2 Max (32GB) | batch=2, len=512 | 4-6 小时 |
| M3 Max (64GB) | batch=4, len=512 | 3-4 小时 |

*相比原数据集（21,476 样本），训练时间约为 1/3*
